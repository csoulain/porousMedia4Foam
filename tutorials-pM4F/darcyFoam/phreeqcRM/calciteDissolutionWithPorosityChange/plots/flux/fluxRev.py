...
##plt.plot(TimeTR, FluxTR,'m',linewidth=5,label='Toughreact')
...
plt.plot(Time, Flux,'c',marker="s",markevery=12,markersize=9,linestyle='None',label='darcy')


#pM4F-dbs Flux results
TimeDBS = []
FluxDBS = []
with open('../B1_dbs/postProcessing/flowRatePatch(name=outlet)/4.73999e+09/surfaceFieldValue1.dat','r') as csvfile:
   plots = csv.reader(csvfile, delimiter=' ')
   for row in plots:
       TimeDBS.append(float(row[0])/(86400*365))
       FluxDBS.append(float(row[1])*100*86400)
plt.plot(TimeDBS, FluxDBS,'k',marker="o",markevery=12,markersize=7,linestyle='None')
