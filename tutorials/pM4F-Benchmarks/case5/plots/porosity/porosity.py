...
fig, ax = plt.subplots()
ax.imshow(im,aspect='auto', extent=(.6,.95,0.005,0.015), zorder=1)
...
plt.plot(DistMIN, Poro300MIN,'r-v',label='MIN3P')

#ToughR
Dist10TR = []
Poro10TR = []
with open('plots/Tough_WebExtraction/Porosity/B3_10y_poro.txt','r') as csvfile:
   plots = csv.reader(csvfile, delimiter='\t')
   for row in plots:
...
Dist100TR = []
Poro100TR = []
with open('plots/Tough_WebExtraction/Porosity/B3_100y_poro.txt','r') as csvfile:
   plots = csv.reader(csvfile, delimiter='\t')
   for row in plots:
...
Dist300TR = []
Poro300TR = []
with open('plots/Tough_WebExtraction/Porosity/B3_300y_poro.txt','r') as csvfile:
   plots = csv.reader(csvfile, delimiter='\t')
   for row in plots:
...
plt.savefig('B3_porosity.eps', format='eps')
plt.show()
