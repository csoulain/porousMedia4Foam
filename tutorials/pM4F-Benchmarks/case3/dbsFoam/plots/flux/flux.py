...
plt.plot(TimeTR, FluxTR,'m-',linewidth=2.5,label='Toughreact')


#pM4F Flux results
Time = []
Flux = []
with open('../darcyFoam/postProcessing/flowRatePatch(name=outlet)/4.73999e+09/surfaceFieldValue1.dat','r') as csvfile:
   plots = csv.reader(csvfile, delimiter=' ')
   for row in plots:
       Time.append(float(row[0])/(86400*365))
       Flux.append(float(row[1])*100*86400)
plt.plot(Time, Flux,'c-',linewidth=2.5,label='pM4F')
...
with open('postProcessing/flowRatePatch(name=outlet)/4.73999e+09/surfaceFieldValue1.dat','r') as csvfile:
   ...
   for row in plots:
       Time.append(float(row[0])/(86400*365))
       Flux.append(float(row[1])*100*86400)
plt.plot(Time, Flux,'k-',linewidth=2.5,label='dbs')
